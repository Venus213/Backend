...
router.post('/create', upload.single("image"), async function (req, res, next) {
    ...
    try {
        ...
        const data = await NAIL.create(req.body);
        // console.log(data);
...
router.put('/update/:id',upload.single("image"), async function (req, res, next) {
    try {
        ...
        u_data = req.body
        req.body.image = req.file.filename
        const datas = await NAIL.findByIdAndUpdate(id, u_data);
        // console.log(datas);
        res.status(201).json({
...
router.get('/show/:id',upload.single("image"), async function (req, res, next) {
    try {      
        id = req.params.id
        const datas = await NAIL.findById(id);
        // console.log(datas);
        res.status(201).json({
            status: "Data Successfully updated!",
            message: "Success",
...
router.delete('/delete/:id', async function (req, res, next) {
    id = req.params.id
    result = req.body
    const data = await NAIL.findByIdAndDelete(id, result);
    // console.log(data);
    try {
        res.status(200).json({
