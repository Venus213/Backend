...
router.post("/create", upload.single("image"), async function (req, res, next) {
  try {
    if (!req.file || !req.body.desc || !req.body.name || !req.body.profession) {
...
router.put(
  ...
  async function (req, res, next) {
    try {
      ...
      u_data = req.body;
      req.body.image = req.file.filename;
      const datas = await CLIENT.findByIdAndUpdate(id, u_data);
      // console.log(datas);
      res.status(201).json({
...
router.get(
  ...
  async function (req, res, next) {
    try {
      id = req.params.id;
      const datas = await CLIENT.findById(id);
      // console.log(datas);
      res.status(201).json({
        status: "Data Successfully updated!",
        message: "Success",
...
router.delete("/delete/:id", async function (req, res, next) {
  id = req.params.id;
  result = req.body;
  const data = await CLIENT.findByIdAndDelete(id, result);
  // console.log(data);
  try {
    res.status(200).json({
