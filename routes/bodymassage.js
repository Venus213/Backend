...
var BODYMASSAGE = require("../model/bodymassage");
...
router.post("/create", upload.single("image"), async function (req, res, next) {
  try {
    ...
    const data = await BODYMASSAGE.create(req.body);
    // console.log(data);
...
router.get("/view", async function (req, res, next) {
  const data = await BODYMASSAGE.find();
...
router.put(
  ...
  async function (req, res, next) {
    try {
      ...
      u_data = req.body;
      req.body.image = req.file.filename;
      const datas = await BODYMASSAGE.findByIdAndUpdate(id, u_data);
      // console.log(datas);
      res.status(201).json({
...
router.get(
  ...
  async function (req, res, next) {
    try {
      id = req.params.id;
      const datas = await BODYMASSAGE.findById(id);
      // console.log(datas);
      res.status(201).json({
        status: "Data Successfully updated!",
        message: "Success",
...
router.delete("/delete/:id", async function (req, res, next) {
  id = req.params.id;
  result = req.body;
  const data = await BODYMASSAGE.findByIdAndDelete(id, result);
  // console.log(data);
  try {
    res.status(200).json({
